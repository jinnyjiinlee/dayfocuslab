  ...
  return (
    <>
      <div className="absolute inset-0 overflow-hidden" aria-hidden="true">
        <div className="absolute top-[-15%] right-[-5%] w-[60vw] h-[60vw] max-w-[700px] max-h-[700px] rounded-full blur-[140px] opacity-25 animate-mesh-1" style={{ background: 'radial-gradient(circle, #085CF0 0%, transparent 70%)' }} />
        <div className="absolute bottom-[-10%] left-[-10%] w-[50vw] h-[50vw] max-w-[600px] max-h-[600px] rounded-full blur-[120px] opacity-20 animate-mesh-2" style={{ background: 'radial-gradient(circle, #713FFF 0%, transparent 70%)' }} />
        <div className="absolute top-[40%] left-[50%] w-[30vw] h-[30vw] max-w-[400px] max-h-[400px] rounded-full blur-[100px] opacity-10 animate-mesh-3" style={{ background: 'radial-gradient(circle, #AFE9FD 0%, transparent 70%)' }} />
      </div>
      <div className="absolute inset-0 opacity-[0.04]" aria-hidden="true" style={{ backgroundImage: 'linear-gradient(rgba(255,255,255,0.1) 1px, transparent 1px), linear-gradient(90deg, rgba(255,255,255,0.1) 1px, transparent 1px)', backgroundSize: '80px 80px' }} />
...
function HeroStats({ t, ko, isInView }: { t: Translations; ko: boolean; isInView: boolean }) {
  const items = [
...
function HeroCTA({ t }: { t: Translations }) {
  return (
  ...
  return (
    <section className="relative min-h-[100dvh] flex items-center overflow-hidden bg-[#050A34]" aria-label={ko ? '메인 히어로' : 'Main hero'}>
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-6 w-full py-32">
        ...
        <div ref={statsView.ref} className="mt-20 md:mt-24 opacity-0 animate-fade-in animation-delay-1000">
          <HeroStats t={t} ko={ko} isInView={statsView.isInView} />
        </div>
      </div>
