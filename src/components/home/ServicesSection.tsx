...
function EnterpriseCard({ t, ko }: { t: Translations; ko: boolean }) {
  return (
    <FadeIn className="mb-4">
      <div className="group relative bg-[#050A34] text-white p-8 md:p-14 rounded-3xl overflow-hidden hover:shadow-2xl hover:shadow-blue-500/10 transition-[box-shadow] duration-500">
        ...
        <div className="relative z-10 grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12">
          ...
          <FeatureGrid features={t.services.enterprise.features} />
        </div>
      </div>
    </FadeIn>
  );
...
const B2C_COLORS = ['#085CF0', '#713FFF', '#9CA3AF'];

function B2CCard({ title, subtitle, desc, price, icon, color, delay }: {
  title: string; subtitle: string; desc: string; price?: string; icon: string; color: string; delay: number;
}) {
  return (
    <FadeIn delay={delay}>
      <div className="group bg-[#F7F9FC] p-7 md:p-8 rounded-3xl border border-gray-100 h-full hover:shadow-xl hover:-translate-y-0.5 transition-[box-shadow,transform] duration-500">
        <div className="flex items-center gap-3 mb-5">
          <div className="w-11 h-11 rounded-xl flex items-center justify-center shrink-0" style={{ backgroundColor: `${color}12` }}>
            <svg className="w-5 h-5" style={{ color }} fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d={icon} />
            </svg>
          </div>
          <div>
            <h3 className="text-lg font-bold text-[#050A34]">{title}</h3>
            <p className="text-xs text-gray-500 font-medium">{subtitle}</p>
          </div>
        </div>
        <p className="text-gray-600 text-sm leading-relaxed mb-4">{desc}</p>
        {price && <p className="text-sm font-bold text-[#050A34]">{price}</p>}
      </div>
    </FadeIn>
  );
}
...
export default function ServicesSection({ t, ko }: { t: Translations; ko: boolean }) {
  const b2cServices = [
    { title: t.services.ax.title, subtitle: t.services.ax.subtitle, desc: t.services.ax.description, price: t.services.ax.price },
    { title: t.services.workshop.title, subtitle: t.services.workshop.subtitle, desc: t.services.workshop.description, price: t.services.workshop.price },
    { title: t.services.studycafe.title, subtitle: t.services.studycafe.subtitle, desc: t.services.studycafe.description, price: undefined },
  ];
  ...
  return (
    <section className="py-16 md:py-28 bg-white relative">
      <div className="max-w-6xl mx-auto px-6">
        ...
        <EnterpriseCard t={t} ko={ko} />

        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {b2cServices.map((svc, i) => (
            <B2CCard key={i} {...svc} icon={B2C_ICONS[i]} color={B2C_COLORS[i]} delay={(i + 1) * 100} />
          ))}
        </div>
